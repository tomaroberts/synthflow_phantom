...
% G_world = G([1,4,5],:); % NB: no need for Mscaling as grad_moments defined: .* (1e-3)^2; %msec^2.mT/m in original synthetic data  
...
%% View
implay_RR([VEL.rl, VEL.ap, VEL.fh],'jet',[0,1]);
implay_RR(VEL3D, 'jet', [0,1] );


%% load original synthetic phantom
Vpad = load_untouch_nii('VEL_VOLUME_srow4_equal0.nii.gz');
implay_RR(Vpad.img(:,:,250:350),'jet',[0,1]);
